...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  document
    ...
    .addEventListener("submit", async function (event) {
      ...
      const productIndex = new URLSearchParams(window.location.search).get(
        "index"
      ); // Get productIndex from URL
...
function displayRatingStats() {
  ...
  for (let i = 5; i >= 1; i--) {
    ...
    const stars =
      '<i class="fas fa-star"></i>'.repeat(i) +
      '<i class="far fa-star"></i>'.repeat(5 - i);
    statsHTML += `
